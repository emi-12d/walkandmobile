...
- (NSArray *) reader:(UIImage *)img {
    // 致命的なクラッシュを防ぐため、色空間を明示的に作成し、Getルールのオブジェクトを解放しないようにします。
    
    // 1. CoreGraphicsの処理に必要な色空間を明示的に作成（メモリを確保したため、後で release が必須）
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();

    CGFloat cols = img.size.width;
    CGFloat rows = img.size.height;

    // 2. RGBA (4チャンネル) のMatを用意
    cv::Mat Img(rows, cols, CV_8UC4);
    cv::Mat image0; // CV_8UC3 (BGR) に変換される Mat

    // 3. CoreGraphicsのコンテキストを作成し、Img.data へ描画
    ...
    // 4. 作成したコンテキストと色空間を解放（Create したもののみ）
    CGContextRelease(contextRef);
    CGColorSpaceRelease(colorSpace); // 自分で Create したため release

    // 5. CGBitmapContextから取り出した RGBA(Img) を OpenCVで扱いやすい BGR(image0) に変換
    cv::cvtColor(Img, image0, cv::COLOR_RGBA2BGR);

    // 6. 元のバッファ Img の解放
    Img.release();
    
    // --- 以下、image0 を使った既存のロジックへ移行 ---
    Mat image = image0;
    // ...
